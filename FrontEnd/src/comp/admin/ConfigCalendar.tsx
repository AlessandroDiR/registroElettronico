    ...
    showHideModal = () => {
        Modal.info({
            ...
            content: <div className="text-justify">
                ...
                <p>Per trovare la chiave API, bisognerà recarsi a <a href="https://developers.google.com/calendar/quickstart/js?hl=it" target="_blank">questo <i className="far fa-external-link"></i></a> indirizzo ed effettuare il login con un qualsiasi account Google. Scendere in basso e cliccare sul bottone <strong>Create API Key</strong> (Crea chiave API), ed attendere il caricamento. Apparirà a schermo una finestra contentente la chiave: basterà copiare il codice ed inserirlo in questa pagina di configurazione.</p>
                <strong>ID del calendario</strong>
                <p>Per trovare l'ID (identificativo) del calendario, basterà effettuare l'accesso su <a href="https://calendar.google.com/" target="_blank">Google Calendar <i className="far fa-external-link"></i></a> con un account che abbia integrato il calendario del corso. Bisognerà poi premere sul simbolo <i className="far fa-cog"></i> in alto a destra e successivamente su <strong>Impostazioni</strong> nel menu che comparirà. Scorrere quindi la lista a sinistra della pagina, e cliccare sul calendario che si desidera integrare. Recarsi quindi sotto la voce <strong>Integra calendario</strong> e copiare (subito in basso) il codice identificativo del calendario (<strong>ID calendario</strong>), per poi incollarlo in questa pagina di configurazione.</p>
